...
class StackOverflow extends StackOverflowInterface with Serializable:
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] =
    ...
    val answers = postings.filter(_.postingType == 2).map(ans => ans.parentId.get -> ans)
                          .partitionBy(new HashPartitioner(8)).persist()


    questions.join(answers).mapValues(Iterable(_)).reduceByKey(_ ++ _)
  ...
  def scoredPostings(grouped: RDD[(QID, Iterable[(Question, Answer)])]): RDD[(Question, HighScore)] =
    ...
    grouped.map(posting => 
      val question = posting._2.head._1
      val answers = posting._2.map(_._2).toArray
  ...
  @tailrec final def kmeans(means: Array[(Int, Int)], vectors: RDD[(Int, Int)], iter: Int = 1, debug: Boolean = false): Array[(Int, Int)] =
    ...
    val newMeansMap: scala.collection.Map[(Int, Int), (Int, Int)] = 
      val grouped = vectors.map(point => findClosest(point, means) -> point).partitionBy(new HashPartitioner(8))
      val reduced = grouped.mapValues(Iterable(_)).reduceByKey(_ ++ _)
      val mapped = reduced.map(mean => mean._1 -> averageVectors(mean._2))
      mapped.collectAsMap()
  ...
  def clusterResults(means: Array[(Int, Int)], vectors: RDD[(LangIndex, HighScore)]): Array[(String, Double, Int, Int)] =
    ...
    val median = closestGrouped.mapValues { vs =>
      ...
      val langCount = vs.groupBy(_._1).map((k, v) => (k, v.size))
      val highest = langCount.toList.sortBy(_._2)
      val langLabel: String   = langs(highest(0)._1 / langSpread) // most common language in the cluster
      val langPercent: Double = ((highest(0)._2).toDouble / vs.size) * 100 // percent of the questions in the most common language
