...
trait DecoderInstances:
  ...
  given Decoder[Int] = 
    Decoder.fromFunction {
      case Json.Num(value) => if value.isValidInt then Some(value.intValue) else None
      case _ => None
    }
  ...
  given Decoder[String] =
    Decoder.fromPartialFunction {
      case Json.Str(value) => value.toString()
    }
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(items) => 
        var decoded = items.map(decoder.decode)
        if decoded.forall(_.isDefined)
        then Some(decoded.map(_.get))
        else None
      case _ => None
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
    Decoder.fromFunction {
      case Json.Obj(mp) => decoder.decode(mp.get(name).get)
      case _ => None
    }
