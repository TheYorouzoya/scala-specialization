...
class KMeans extends KMeansInterface:
  ...
  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] =
    var pmap = points.groupBy(point => findClosest(point, means))
    pmap = means.map(point => point -> pmap.getOrElse(point, ParSeq.empty[Point])).toMap
    pmap
  ...
  def update(classified: ParMap[Point, ParSeq[Point]], oldMeans: ParSeq[Point]): ParSeq[Point] =
    for point <- oldMeans
    yield findAverage(point, classified.getOrElse(point, ParSeq.empty[Point]))

  def converged(eta: Double, oldMeans: ParSeq[Point], newMeans: ParSeq[Point]): Boolean =
    oldMeans.zip(newMeans).map((oldMean, newMean) => oldMean.squareDistance(newMean) <= eta)
      .fold(true)(_ && _)
