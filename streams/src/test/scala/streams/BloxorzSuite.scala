...
class BloxorzSuite extends munit.FunSuite:
  ...
  test("neighborsWithHistory test") {
    new Level1:
      ...
      assertEquals(neighborsWithHistory(Block(Pos(1,1),Pos(1,1)), List(Left,Up)), result)
  }
  ...
  test("newNeighborsOnly test") {
    new Level1:
      ...
      var explored =  Set(Block(Pos(1,2),Pos(1,3)), Block(Pos(1,1),Pos(1,1)))
      var result = Set(
          (Block(Pos(2,1),Pos(3,1)), List(Down,Left,Up))
        ).to(LazyList)
      assertEquals(newNeighborsOnly(neighbors, explored), result)
  }
