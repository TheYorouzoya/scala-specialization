...
case class Fork(
  ...
  def insert(b: Body): Fork =
    if b.x < centerX then
      ...
      if b.y < centerY 
      then Fork(nw, ne.insert(b), sw, se)
...
extends Quad:
  val (mass, massX, massY) = {
      val result = bodies.foldLeft((0.0f, 0.0f, 0.0f))(
        (acc, b) => (acc._1 + b.mass, acc._2 + b.mass * b.x, acc._3 + b.mass * b.y))
      (result._1, result._2 / result._1, result._3 / result._1)
    }
  val total: Int = bodies.length
  def insert(b: Body): Quad =
    if size <= minimumSize
    then Leaf(centerX, centerY, size, bodies.+:(b))
    else 
      var empQuad = Fork(
        ...
        Empty(centerX - (size / 4), centerY + (size / 4), size / 2),
        Empty(centerX + (size / 4), centerY + (size / 4), size / 2))
      bodies.foreach(bdy => empQuad = empQuad.insert(bdy))
      empQuad.insert(b)
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface:
  ...
  def +=(b: Body): SectorMatrix =
    ...
    matrix(sectorX + sectorY * sectorPrecision).+=(b)
    this

  def apply(x: Int, y: Int) = matrix(y * sectorPrecision + x)

  def combine(that: SectorMatrix): SectorMatrix =
    for (index <- 0 until that.matrix.length) {
      this.matrix(index) = this.matrix(index).combine(that.matrix(index))
    }
    this
