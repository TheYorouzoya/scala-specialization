...
class Simulator(val taskSupport: TaskSupport, val timeStats: TimeStatistics):
  ...
  def updateBoundaries(boundaries: Boundaries, body: Body): Boundaries =
    val newBoundary = new Boundaries()
    newBoundary.minX = if body.x < boundaries.minX then body.x else boundaries.minX
    newBoundary.maxX = if body.x > boundaries.maxX then body.x else boundaries.maxX
    newBoundary.minY = if body.y < boundaries.minY then body.y else boundaries.minY
    newBoundary.maxY = if body.y > boundaries.maxY then body.y else boundaries.maxY
    newBoundary
    

  def mergeBoundaries(a: Boundaries, b: Boundaries): Boundaries =
    val mergedBoundary = new Boundaries()
    mergedBoundary.minX = if a.minX < b.minX then a.minX else b.minX
    mergedBoundary.maxX = if a.maxX > b.maxX then a.maxX else b.maxX
    mergedBoundary.minY = if a.minY < b.minY then a.minY else b.minY
    mergedBoundary.maxY = if a.maxY > b.maxY then a.maxY else b.maxY
    mergedBoundary
  ...
  def computeSectorMatrix(bodies: coll.Seq[Body], boundaries: Boundaries): SectorMatrix = timeStats.timed("matrix") {
    val parBodies = bodies.par
    parBodies.tasksupport = taskSupport
    parBodies.aggregate(SectorMatrix(boundaries, SECTOR_PRECISION))(_+=_, _.combine(_))
  }
